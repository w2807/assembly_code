...
boot:
    jmp boot_begin
fun1   db '1) reset pc',0
fun2   db '2) start system',0
fun3   db '3) clock',0
fun4   db '4) set clock',0
    ;相减得到的是标号的相对位置，+7e00h得到的绝对位置
fun_pos     dw offset fun1 - offset boot + 7e00h
            dw offset fun2 - offset boot + 7e00h
            dw offset fun3 - offset boot + 7e00h
            dw offset fun4 - offset boot + 7e00h
time    db 'YY/MM/DD hh:mm:ss',0
readtime    db 9,8,7,4,2,0
clock1    db 'press F1 to change the color    press ESC to return menu',0
clock2    db 'input Date and Time,(YYMMDDhhmmss):',0
...
get_time_info:
    ...
    mov cx,6
    ;获取存放单元地址
    mov bx,offset readtime - offset boot + 7e00h
    ;通过替换来显示
...
show_menu:
    ...
    mov di,160*10+30*2
    ;保存在直接定址表的绝对位置
    mov bx,offset fun_pos - offset boot + 7e00h
    ;菜单有5行
    mov cx,5
s1:
    ;这里相当于外循环，每次一行
